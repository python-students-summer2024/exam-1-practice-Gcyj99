...
def weather_helper():
  ...
  if temperature < 40:
        snowing = input("Is it snowing? (yes/no): ").strip().lower()
        if snowing in ['yes', 'yeah', 'yup']:
            wearing_jacket = input("Are you wearing a warm jacket? (yes/no): ").strip().lower()
            ...
            if raining in ['yes', 'yeah', 'yup']:
                have_umbrella = input("Do you have an umbrella? (yes/no): ").strip().lower()
                if have_umbrella in ['yes', 'yeah', 'yup']:
                    print("Good job staying dry!")
                else:
                    print("You must enjoy getting wet!")
  elif temperature > 90:
        have_ac = input("Do you have air conditioning? (yes/no): ").strip().lower()
        if have_ac in ['yes', 'yeah', 'yup']:
            print("Stay cool indoors.")
        else:
            print("I hope you have a fan.")
  elif temperature < -70 or temperature > 134:
